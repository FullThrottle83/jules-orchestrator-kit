...
- **Our Difference:** We ship a **zero-dependency stdio MCP server** (`src/mcp.mjs`, accessible via `agentctl mcp`) to enable direct embedding inside Claude Desktop, Cursor, and Antigravity without port conflicts or third-party server dependencies.
...
- **Our Difference:** Our kit exposes the OODA loop in the CLI (`agentctl gate --fix`), making it possible to execute the exact same logic locally or in CI pipelines.
...
- **Our Difference:** We integrate nonced `<<<UNTRUSTED-DATA-BEGIN>>>` fences directly in `src/prompt-guard.mjs` alongside pre-flight static analysis layering, ensuring zero third-party dependencies.
...
- **Atomic Budgeting:** A local ledger system (`.agent/state/ledger-<date>.jsonl`) that manages daily session budgets and blocks runaway loops.
