...
- **Contributor Provenance & Attribution (P13) (`CONTRIBUTORS.md`, `CONTRIBUTING.md`)**: Formalized maintainer and autonomous agent attribution (82 human maintainer commits, 366 autonomous coding agent commits) with conventional commits and release gates documentation.
- **Cold-Start Onboarding Hardening (P12) (`docs/cold-start-trial.md`, `src/git.mjs`, `src/ops/doctor-registry.mjs`, `src/wizard-task.mjs`)**: Filtered untracked root package artifacts during task creation on newly initialized repos, updated `init` commit hints to include package files, and made `agentctl doctor` red-flag missing verification oracles when verification is required.
- **Trojan Source BiDi Override Detection (PR #23) (`src/security.mjs`, `test/trojan-source.test.mjs`)**: Implemented `checkTrojanSource` to detect invisible Unicode bidirectional control characters (`U+202A`–`U+202E`, `U+2066`–`U+2069`, `U+061C`, `U+200E`, `U+200F`) on added code lines in diffs (CVE-2021-42574). Wired into `scanDiff` to emit a `CRITICAL` finding (`TROJAN_SOURCE_DETECTED`) for source files while exempting markdown documentation.
- **Positional Task Envelope & Queue Auto-Selection Dispatch (`bin/agentctl.mjs`)**: `agentctl dispatch` now accepts positional task envelope paths (e.g. `.agent/jules-queue/TASK-123.md` or `TASK-123`), automatically inherits embedded metadata defaults (title, role, tier, verification command, and flags) via `parseEnvelopeHeader`, and auto-consumes the next pending queue task when invoked without arguments. Dispatched queue tasks are automatically moved to `.agent/jules-queue/completed/` upon successful execution.
- **Testing Specialist Role (`.agent/prompts/Testing.md`, `src/role-resolver.mjs`)**: Added canonical `testing` role (with aliases `test`, `unit-test`, `integration-test`, `qa`) dedicated to unit and integration test quality, negative-path coverage, deterministic fixtures, state isolation, and mutation sensitivity without test weakening.
...
### Changed
- **Security Module Split (P05) (`src/security.mjs`, `src/fs-atomic.mjs`, `src/scope-guard.mjs`, `src/secret-scanner.mjs`, `src/test-tamper-guard.mjs`, `src/bidi-guard.mjs`)**: The 3,425-line `src/security.mjs`, which bundled five responsibilities, is now a facade over five focused submodules — atomic filesystem writes, scope policy and the regex-free glob matcher, credential/PII detection, test-tamper detection with its multi-language statement parser, and Trojan Source BiDi detection. `scanDiff()` remains in the facade as the orchestrator that walks the diff once and calls them. Pure relocation: no regex, signature, or control flow changed, and the facade re-exports all 26 public symbols so every existing `import … from "./src/security.mjs"` path and every `index.mjs` export still resolves to the same object. The 59-canary `guard-reach` activation gate stays red-if-silenced across the new module boundaries.
- **Role Prompt Consolidation & One-Directional Aliases (P03) (`.agent/prompts/`, `src/role-resolver.mjs`)**: Deleted the 9 legacy duplicate prompt files after porting their unique instructions into the canonical roles; `.agent/prompts/` now holds exactly the 12 canonical roles plus `Task_Template.md`. `ROLE_ALIASES` is strictly one-directional (legacy/shortcut → canonical) with the reverse map moved to a private on-disk fallback so pre-consolidation checkouts keep resolving.
- **Audit Ledger Bypass Closed (P04) (`src/budget.mjs`, `src/state.mjs`, `test/budget-ledger.test.mjs`)**: Closed verification bypass where missing daily budget ledgers were silently skipped; retired legacy `scripts/utils.mjs` shim and relocated core business logic into `src/`.
- **Security Invariants & Threshold Grounding (P09) (`SECURITY.md`)**: Corrected Shannon entropy thresholds (`> 3.6` bits/char for environment variables, `> 4.5` bits/char for base64/hex tokens) and accurately documented execution envelope parameters and ledger chain verification.
- **Test Archaeology & Domain Suite Consolidation (P08) (`test/`)**: Consolidated 16 scattered audit-trial test files into 4 domain-focused suites (`cli-surface.test.mjs`, `budget-ledger.test.mjs`, `tamper-detection.test.mjs`, `provider-failover.test.mjs`), expanding test coverage to 1,530 passing unit tests across 204 suites.
- **Cross-Platform CRLF Normalization**: Normalized line endings across Windows, Linux, and macOS in reference generators and doc-sync gates.
- **Router Role Normalization & Tier Alignment (`src/router.mjs`)**: Normalized requested roles via `ROLE_ALIASES` so canonical roles route identically to legacy aliases: `security` forces complex tier, `auditor` leans complex, and `hygiene` / `performance` lean fast.
- **Dispatch Idempotency & Final Payload Ceiling (`src/engine.mjs`)**: Idempotent role injection prevents duplicate specialist prompt headers, preserves role/memory context when `taskInstructions` is present, propagates active config overrides to role hydration, and validates the assembled final prompt against `limits.promptKb`.
- **Code Block & Inline Backtick Preservation in Vocabulary Sanitizer (`src/prompt-guard.mjs`)**: `sanitizePromptVocabulary` now preserves fenced code blocks and inline backtick code spans verbatim, preventing alteration of exact technical commands, signals (`SIGKILL`), and identifiers.
- **Task Objective Preflight Validation (`src/wizard-task.mjs`)**: `planTaskCreate` now enforces that a concrete task objective or template is provided before injecting role instructions, preventing creation of empty tasks with only role descriptions.
- **Two-Hop Legacy Alias Resolution (`src/role-resolver.mjs`)**: Shortcuts like `perf` now map through canonical `performance` to legacy `Bolt.md` in legacy-only checkouts.
- **Mutation Template Invariant (`src/web-templates.mjs`)**: In `agent-qa-mutation`, replaced the directive to delete surviving tests with a mandate to add targeted assertions and classify equivalent mutants, strictly forbidding test deletion.
- **Specialist Prompts Hardening (`.agent/prompts/`)**: Clarified retry and fallback semantics in `Resilience.md`, decoupled secret mechanisms from Node-specific `process.env` in `Security.md`, forbade permissive coercions and casts in `Types.md`, and expanded `Docs.md` to cover technical documentation, API references, and CLI manuals.
- **Engineering Role Professionalization & Aliases (`src/role-resolver.mjs`, `.agent/prompts/`)**: Replaced fantasy/RPG nomenclature across all 12 specialist roles (`Bulwark` $\rightarrow$ `resilience`, `Alchemist` $\rightarrow$ `database`, `Janitor` $\rightarrow$ `hygiene`, `Sentinel` $\rightarrow$ `security`, `Bolt` $\rightarrow$ `performance`, `Spectator` $\rightarrow$ `e2e`, `Overseer` $\rightarrow$ `auditor`, `Typist` $\rightarrow$ `types`, `Scribe` $\rightarrow$ `docs`, `Hunter` $\rightarrow$ `debugger`, `Testing`, `a11y` $\rightarrow$ `a11y`). Stripped emojis and colloquial prose, while maintaining complete backward compatibility via `ROLE_ALIASES` for CLI flags, dispatches, and configs.

### Fixed
- **Dashboard Port Range Validation & Consensus (D1, D2, D3) (`bin/agentctl.mjs`, `src/dashboard.mjs`, `src/ops/command-registry.mjs`, `README.md`)**: Supported both positional `agentctl dashboard <port>` and `--port <port>` syntax, added 1024–65535 port range validation with clean exit code 1 errors on invalid input instead of unhandled exceptions, ensured `createDashboardServer` honors configured ports, and unified default port consensus at 4100 across the CLI, command registry, and documentation.
- **Dynamic Guardrails Word Boundaries & File Loading (D4, D5, D6) (`src/dispatch.mjs`, `scripts/jules-dispatch.mjs`, `.agent/rules/dynamic-guardrails.json`)**: Loaded custom project rules dynamically from `.agent/rules/dynamic-guardrails.json`, enforced strict regex word boundaries (`\bsec\b`, `\bkey\b`, `\bauth\b`) to eliminate false-positive activations on words like "breakfast" or "sector", and updated trigger labels to canonical specialist role identifiers.
- **Preflight Static Manifest & Syntax Validation (D7) (`src/dispatch.mjs`, `scripts/jules-dispatch.mjs`)**: Replaced unconditional `"PASSED"` stub in `runPreflightStaticCheck` with verification of `package.json` existence and JSON object parse integrity, failing closed when manifests are missing or malformed.
- **Sterile Process Management Vocabulary Case Sensitivity (D8) (`src/prompt-guard.mjs`, `test/prompt-guard.test.mjs`)**: Added `/i` flag to `/\bSIGKILL\b/gi` in `STERILE_VOCABULARY_MAP`, ensuring clinical process term substitution catches lowercase and mixed-case process termination tokens.
- **TODO Scanner Safety Limits & Gitignore Exclusions (D9) (`src/todo-scanner.mjs`, `scripts/jules-scan-todos.mjs`)**: Enforced safe traversal bounds on `agentctl scan` by restricting scanning to source extensions (`.js`, `.mjs`, `.ts`, `.py`, `.rs`, `.go`, `.md`), imposing a 1 MB per-file size ceiling, skipping binary buffers with null bytes, and honoring root `.gitignore` exclude patterns.
- **Dispatch Argv Scope Isolation (D10) (`src/dispatch.mjs`, `scripts/jules-dispatch.mjs`)**: Moved CLI argument parsing inside the execution entry guard (`process.argv[1]`), preventing module importers from prematurely consuming or baking global `process.argv` into function defaults.
- **Dashboard Manifest Read Deduplication (D11) (`src/dashboard.mjs`)**: Replaced redundant `readFileSync` of `package.json` in the dashboard server with imported `KIT_VERSION`.
- **Composite GitHub Action API Key Input (D12) (`.github/actions/setup-jules/action.yml`)**: Parameterized `jules_api_key` as an explicit action input defaulting to `${{ secrets.JULES_API_KEY }}`, preventing silent authentication failures caused by unpopulated composite `env:` blocks.
- **Asset Integrity Scanner Non-Vacuous Status Reporting (D13) (`src/asset-integrity.mjs`, `scripts/asset-integrity-check.mjs`)**: Returned explicit `status: N/A` when no asset directories exist, and verified fail-closed detection when corrupt image headers are encountered.
- **Specialist Role Consolidation & One-Directional Aliases (D14, D15) (`.agent/prompts/`, `src/role-resolver.mjs`)**: Deleted 9 legacy duplicate role prompt files after porting unique domain directives into 12 canonical roles, and made `ROLE_ALIASES` strictly one-directional (legacy $\rightarrow$ canonical) with private fallback for historical checkouts.
- **Balanced Swarm Slot Partitioning (D16) (`src/dispatch.mjs`, `scripts/jules-dispatch.mjs`)**: Replaced unbalanced slot partitioning with `getAlphaRange`, partitioning A–Z into contiguous letter ranges differing by at most one character for arbitrary slot counts $N > 2$.
- **Execution Envelope Config Scope Hashing (D17) (`src/execution-envelope.mjs`, `test/execution_envelope.test.mjs`)**: Aligned `configSha` generation to hash `{ denyPaths, allowPaths, protectPaths }` consistently, establishing tamper-evident config scope tracking across envelopes.
...
## Archived Roadmap Milestone Summaries (v0.20.0 – v0.65.0)

Curated milestone summaries relocated verbatim from `ROADMAP_V1.md` during the P10 documentation
consolidation, so the roadmap keeps only the last 10 shipped versions. These are archival highlights,
not release entries: the per-release notes for every version above (Keep a Changelog format,
`## [x.y.z] - date` headings) are the canonical history and remain untouched.
