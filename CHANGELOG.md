...
### Changed
- **Security Module Split (P05) (`src/security.mjs`, `src/fs-atomic.mjs`, `src/scope-guard.mjs`, `src/secret-scanner.mjs`, `src/test-tamper-guard.mjs`, `src/bidi-guard.mjs`)**: The 3,425-line `src/security.mjs`, which bundled five responsibilities, is now a facade over five focused submodules — atomic filesystem writes, scope policy and the regex-free glob matcher, credential/PII detection, test-tamper detection with its multi-language statement parser, and Trojan Source BiDi detection. `scanDiff()` remains in the facade as the orchestrator that walks the diff once and calls them. Pure relocation: no regex, signature, or control flow changed, and the facade re-exports all 26 public symbols so every existing `import … from "./src/security.mjs"` path and every `index.mjs` export still resolves to the same object. The 59-canary `guard-reach` activation gate stays red-if-silenced across the new module boundaries (59/59 canaries, 1,524 tests across 204 suites, 0 lint errors).
- **Role Prompt Consolidation & One-Directional Aliases (P03) (`.agent/prompts/`, `src/role-resolver.mjs`)**: Deleted the 9 legacy duplicate prompt files after porting their unique instructions into the canonical roles (Sentinel's `exec()` coverage into `Security.md`, Scribe's cross-surface parity rule into `Docs.md`, Typist's untrusted-source enumeration into `Types.md`); `.agent/prompts/` now holds exactly the 12 canonical roles plus `Task_Template.md`. `ROLE_ALIASES` is strictly one-directional (legacy/shortcut → canonical) with the reverse map moved to a private on-disk fallback so pre-consolidation checkouts keep resolving; `--role bolt` / `--role sentinel` keep working via aliases. CLI help, examples, the flaky-healer default role (`janitor` → `hygiene`), and `JULES_RULES_TEMPLATE.md` now name canonical roles only.
