...
## [Unreleased]
*The policy the gate obeys, and the revision it attests.*

The fifth cold-start trial, against v0.71.0. Six findings, and they share one root twice over: nothing separated the policy the gate obeyed from the diff the gate was judging, and nothing separated the code the gate ran from the revision it attested. F06, F08, F10 and the staged half of F07 were independently confirmed; all six are reproduced in `test/trusted-policy.test.mjs`, each with a case that fails against v0.71.0.

### Fixed
- **One Trusted-Input Resolver (`src/trusted-policy.mjs`, `src/engine.mjs`) — F06, F07, F08**: `verify.tamperGuard` was read from the base commit, with a comment explaining why. `verify.test`, `verify.profile`, `base_branch`, `minTests`, `required`, the stage list and the protected-path lists were read from whichever config `loadConfig` found on disk. That split is three separate findings. **F07**: an agent staged a broken `is.go`, left a no-op `verify.test` *unstaged*, and `--mode staged` executed it — the path list was checked, the file's contents were not, and the stage plan came from the live config because `loadConfig` had already frozen the working tree's expanded `verify.profile` into `config.verify.stages`. **F08**: committing `base_branch: HEAD` alongside the change made `--mode committed` compare HEAD with HEAD, so the policy author and the trusted-policy selector were the same untrusted diff. **F06**: on a repository whose base has no kit config there was no trusted policy at all, and the gate obeyed the uncommitted scaffold the same diff had rewritten. Every field the gate obeys is now resolved in exactly one place, from a commit the diff cannot author, and `TRUSTED_FIELDS` in `src/guard-policy.mjs` is the row-per-field contract — a hostile value for each is written into the working tree and must not reach the resolver. Committed mode additionally refuses a base that resolves to HEAD, before anything is read *from* that base: a gate approval is a comparison, and a run with nothing to compare is not one.
- **First Install Accepts Only The Scaffold `init` Wrote (`src/trusted-policy.mjs`, `src/wizard-init.mjs`) — F06**: the bootstrap exception exists because a gate that rejects its own installation is broken, and that reasoning is still right. But accepting the scaffold's *presence* was read as accepting its *contents*, so an agent supplied its own oracle and was approved. The scaffold is now accepted only when it is byte-identical to what `init` generates for that repository, regenerated from the repository itself via `planInit(root, { pristine: true })` — seeded from nothing in `.agent/**`, or the comparison would read the agent's rewritten command back as the expected value and match itself. A modified scaffold is refused and the output names the file and the field: `.agent/config.yml → verify.test is "node -e \"process.exit(0)\"" in the uncommitted scaffold, but \`init\` generates "npm test" for this repository.` Fields the gate never consults — `provider`, `tier`, `presets` — are carried over rather than regenerated, because they are answers the operator gave the wizard and failing an honest install over one would be the same first-run rejection of correct code.
- **Staged And Committed Modes Verify The Revision They Judge (`src/revision-snapshot.mjs`, `src/engine.mjs`) — F10**: both modes read the diff from the index or from a commit and then ran the verification command in the repository root. Staging a broken file and restoring the healthy one on disk produced `APPROVED` for a snapshot whose suite fails — two different copies of the code, one verdict, with no config change and no escape flag involved. The snapshot is now materialised into a temporary directory (`git checkout-index` for the index, `git archive` for a commit) and the stages run there; if it cannot be materialised the gate refuses and says why, because attesting a revision you did not execute is the failure this tool exists to refuse. Working-tree mode is unchanged and still runs in the root, because there the root *is* the revision. Gitignored dependency directories — `node_modules`, `.venv`, `target` and the rest — are linked back in, and only when git agrees they are untracked: a suite failed for a missing toolchain is correct work rejected, and a `vendor/` the repository actually tracks must come from the snapshot or F10 returns by the back door.
- **A Command That Cannot Fail Is Not An Unlisted Runner (`src/stack-detector.mjs`) — F09**: `isPlaceholderTestScript` already refused `true`, `:` and an interpreter handed an empty program. `node -e "process.exit(0)"`, `sh -c :`, `pytest --collect-only` and `go test -run "^$" ./...` are the same class one step along, and each approved broken production code in the trial. They are refused now, and `describeIncapableTestCommand` names which of the two reasons applies so the output says *the program the interpreter runs cannot fail* rather than only that something was wrong. The collection floor stays one-sided and untouched: it is the rule for a runner whose dialect is unrecognised, and this is the rule for a command that is recognisably not one. `CAPABLE_ORACLES` is the counterweight — `go test -run TestFoo` narrows to a real subset, `pytest -k "not slow"` is how a large suite is driven, and `node -e` with a body that does work is unusual but honest.
- **A Passing Suite Must Be About The Code In The Diff (`src/source-binding.mjs`) — F11**: with the package installed as a wheel and a configured `python3 -m pytest`, a src-layout repository imports from site-packages: `-m` puts the working directory on `sys.path`, `src/` is not the working directory, so 297 tests passed against the installed library while the working source returned the wrong result and the gate approved the source diff. v0.71.0 fixed what `init` *chooses*; a command already sitting in a config was untouched, and the report is careful about that distinction. So the rule cannot be "the command lacks PYTHONPATH" — an editable install, `pythonpath` in `pyproject.toml` and a `conftest.py` at the root all bind the source correctly and rejecting them would fail work that is right. The gate asks the interpreter instead: it runs the configured command's interpreter, in the same directory and environment the stage will use, and prints where each *edited* package actually resolves from. A package the diff edits that resolves outside the audited source is reported with the path it came from and the three ways to bind it. Anything the probe cannot establish — no interpreter, an import error, a stack it cannot read — yields nothing, because absence of evidence is not evidence.
