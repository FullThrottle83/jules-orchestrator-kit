...
*An end-to-end simulation of the advertised workflow — `init`, `task create`, `gate`, `queue` — in a clean repository. Nothing below was found by reading the code; each defect is something the run did in front of us. Two of them made the kit unusable on a first install, and the rest are the same shape: a component computed the answer, and the layer above it threw the answer away.*

#### Fixed
- **A lockfile bump failed closed as a CRITICAL secret leak (`src/security.mjs`)**: `decodeBase64Blobs` counted every token matching the base64 alphabet against a 64-payload cap and failed closed on overflow. A sha256 hex digest is 64 characters of that alphabet; it decodes to binary and is discarded, but it still consumed a slot. Any diff with 65 or more integrity hashes — the dependency bump this kit advertises as an ideal task — was rejected as a leak with no credential in it. The cap now counts only blobs that survive the printable-ratio check, with separate bounds on tokens examined and bytes decoded, so the budget still protects the gate without spending itself on data it already cleared.
- **`agentctl init` left the kit's own bookkeeping in the working tree (`src/scaffold.mjs`, `bin/agentctl.mjs`, `bin/init.js`)**: the gate audits that tree, so every ledger, evidence manifest and telemetry line the kit wrote came back as a diff the agent was accused of making — first as a scope violation, then, once enough evidence files accumulated, as a CRITICAL secret verdict. A new user met both before dispatching a single task. Init now writes the runtime `.gitignore` entries, including `.agent/jules-queue/*.md` with the `README.md` negation after it, so the first gate following `task create` no longer rejects the tree for the envelope `task create` just wrote.
- **The two init paths scaffolded different repositories (`src/scaffold.mjs`)**: `jules-init` wrote `AGENTS.md`, the role prompts and the guardrails; `agentctl init` — the one the README's quickstart points at — wrote neither. Anyone following the quickstart got a Jules that never saw the protocol and a `--role` flag with nothing to resolve against. Both entry points now call one `scaffoldRepoAssets()`; `bin/init.js` keeps only its own extras and lost ~95 lines of divergent duplication. Existing files are preserved unless `--force`, and an `AGENTS.md` that already exists is appended to rather than replaced.
- **`agentctl queue` reported success for a queue that dispatched nothing (`bin/agentctl.mjs`)**: a run where every task was rejected — no API key is the common one — still printed `Processed 3 task(s).` and exited `0`. Neither an operator nor a CI job could tell a dispatched queue from a dead one. The failures were in the result object the whole time; only `--json` ever showed them. `queue` and `swarm` now share one reporter that names each failed task and its cause, notes that failures stay queued for a re-run, and exits `1`. `swarm` also printed the literal string `undefined` for tasks it had no count for.
- **A secret finding would not say where the secret was (`src/security.mjs`)**: `scanDiff` joined every added line in the diff into one string before scanning, so a finding could only ever report that a credential existed somewhere in the patch. Added lines are now grouped by file and scanned per file, carrying the post-image line number from the `@@` headers; a flagged group gets a second per-line pass to pin the number down, and reports the file alone when no single line reproduces the verdict, rather than pointing at an innocent line. The joined text is still scanned as a fallback when every file comes back clean, so nothing that only matches across a file boundary was traded away for the attribution. Side effect worth having: the decode budget is now per file instead of per diff, so a lockfile early in a patch can no longer exhaust it and leave later files undecoded.
- **The gate could not run a package-manager command on Windows (`src/git.mjs`)**: `runCmd` spawned a tokenized command with `execFileSync(..., { shell: false })`, and every package-manager entry point on Windows is a `.cmd` shim that cannot be spawned that way. `npm test` — the kit's own default verify command — failed with `spawnSync npm ENOENT` on every Windows install, so the VERIFY phase had never actually run there; it reported a plain non-zero verification and, before the change below, printed nothing that would have identified the cause. Found by the new diagnostics, on the CI matrix, in this release. The tokenized path now spawns through the shell on Windows, which is lossless precisely because those tokens came from splitting on whitespace; an array command, whose elements may contain spaces, is excluded.
- **A failed VERIFY phase printed nothing but `❌ FAIL` (`src/engine.mjs`, `bin/agentctl.mjs`)**: it is the one gate phase whose failure the operator has to fix in their own code, and the only one that reported no diagnostics at all. The command's stdout and stderr were captured, redacted and hashed into the evidence manifest, then discarded before anyone could read them. The phase now carries a `failure` record — stage, command, exit code, both streams, diagnostics — which the CLI renders with a bounded tail and `--json` exposes unchanged.
- **Remediation hints keyed on the exit code, which is ambiguous (`bin/agentctl.mjs`)**: exit `3` is also what a `strictTestLock` tamper verdict returns, so a rewritten test file was answered with advice about `--allow-protected`. Hints now key on the phase that actually failed. A run that already passed `--fix` is no longer told to try `--fix`, and a flaky quarantine (exit `8`) gets its own hint instead of nothing — re-running cannot clear that verdict.
- **`dispatch --dry-run` was indistinguishable from a real dispatch (`bin/agentctl.mjs`)**: it printed the same `✅ Task Dispatched Successfully!` banner, down to a placeholder session id, so a rehearsal read as work in flight and the operator waited for a session that was never going to exist.
- **`agentctl rollback --latest` was rejected as an unknown option (`bin/agentctl.mjs`)**: `restoreCheckpoint()` accepted the string all along; the flag was simply never declared to `parseArgs`.
- **`dispatch --role` silently downgraded an unresolvable role (`bin/agentctl.mjs`, `src/engine.mjs`)**: `task create --role` has always rejected one, so the two commands disagreed about what the same flag meant and a typo handed the work to a generic agent. A role typed on the command line is a statement of intent and now fails closed. A role arriving from a task envelope or internal synthesis still warns and proceeds — killing an automated flaky-heal swarm over a missing prompt file helps no one.
- **The optimizer suggested a flag that does not exist (`src/task-optimizer.mjs`)**: `MISSING_ORACLE` told the operator to pass `--verify`; the flag is `--verify-cmd`.

#### Changed
- **Every command that takes a prompt now takes it the same three ways (`bin/agentctl.mjs`)**: `dispatch` accepted a flag, a file or a positional; `task create` accepted only `--prompt`; `task optimize` accepted only a positional. The form learned on one command failed on the next — loudly on `task create "do the thing"`, which reported a missing prompt while holding one, and silently on `task optimize --prompt "..."`, which scored an empty string. All three resolve it through one helper, and a `--prompt-file` that does not exist is now an error rather than an empty prompt. On `task optimize` the file flag has no short form, because `-f` is already `--fix` there.
- **`agentctl init` ends with the next step rather than a bare success line (`bin/agentctl.mjs`)**: it now reports what it scaffolded, including the commit step, so the recommended first gate runs against a committed baseline.
...
*The wizard defect arrived from an external installation. Everything else here is what auditing the same shape turned up: a control that exists, is documented, and does nothing.*

#### Breaking
- **`FALLBACK_TIER` is now `free`, was `ultra` (`src/config.mjs`)**: a repository with no `tier:` — every repository until someone sets one — was granted a 300-task allowance and 60 concurrent workers. The two errors are not symmetrical. Guessing low costs a paid operator throughput until they add one line; guessing high dispatches a Free account straight into `FAILED_PRECONDITION` quota rejections on its first real run, after the kit has already reported the work as scheduled. **Migration:** add `tier: pro` or `tier: ultra` to `.agent/config.yml`. `agentctl budget` shows which tier is in force and where it came from.

#### Fixed
- **Interactive `agentctl task create` discarded every answer (`src/wizard-task.mjs`, `src/wizard-init.mjs`)**: both wizards spread `...options` **last** when handing off to their planning function. `bin/agentctl.mjs` builds that object from `parseArgs`, which puts every *declared* option on it — unpassed flags as present keys holding `undefined` — and a spread of `{prompt: undefined}` overwrites, because the key exists. Every answer the user had just typed was replaced immediately before validation, so the command died on `Task prompt cannot be empty` no matter what was entered. Interactive `task create` has been broken since v0.29.0; only the flag form (`-t` / `-p`) ever worked. The spread now comes first, which preserves flag values while letting an interactive answer win.
- **`agentctl init` ignored the tier picked from the menu (`bin/agentctl.mjs`, `src/wizard-init.mjs`)**: the same defect, with a hardcoded `values.tier || "pro"` on top of it. Selecting Free or Ultra still wrote `tier: pro`, and a re-run overwrote the tier already recorded in `.agent/config.yml` — so a Free account was scaffolded with 100 daily tasks and 8 workers, and the failure surfaced later as quota rejections rather than as a wrong answer at the prompt.
- **The CI scope guard could not block anything (`.github/workflows/agent-scope-guard.yml`, `scripts/ci-scope-guard.mjs`)**: it parsed `.agent/protected-paths.json` with `sed`, and under `bash -e` a failing command substitution inside an assignment aborts the step before a single comparison runs — so the job reported green on every pull request it had ever run on. Its hand-rolled matcher also disagreed with `src/security.mjs` (no case folding), and `git diff --name-only` without `-z` splits any path containing a space or a non-ASCII character into two names that match nothing. Replaced with a Node implementation calling the same `checkScope()` the local gate uses. The manifest is now read from the pull request's **base** SHA, so a branch can no longer delete its own guard; an unreadable manifest fails the job instead of passing it; and the `allow-protected-paths` label documented in `AGENTS.md` is finally implemented.
- **Three stacked fail-opens in `pr harvest --auto` (`src/ops/pr-harvest.mjs`)** — the only irreversible thing the kit does unattended. A pull request with no status checks at all scored `passing: true`, because `every()` on an empty array is `true`. `diffLines` was never passed to `classifyRiskTier`, making the R2 large-diff threshold unreachable from this path. And a missing or truncated file list graded `R0_COSMETIC` — the most permissive tier — off exactly the evidence whose absence should have blocked the decision.
- **Pull request review comments reached a dispatch prompt unfenced (`src/review-repair.mjs`)**: `createReviewRepairTask` interpolated `comment.body` — third-party text, from anyone who can comment — directly into the prompt. The kit already shipped `sanitizeUntrustedData()`; this path simply never called it.
- **The guardrail footer named the wrong manifests (`src/wizard-task.mjs`)**: it told the agent `package.json, pnpm-lock.yaml, tsconfig.json` were protected while `BUILTIN_PROTECT` already listed `Cargo.toml`, `go.mod` and `pyproject.toml` for the gate. The kit knew the right answer and told the agent a different one, so an agent could edit a file the gate would then reject — a wasted task and a confusing rejection. The footer is now derived from the repository's own scope.
- **`fail-fast` erased the compatibility matrix (`.github/workflows/jules-audit.yml`)**: a matrix exists to answer "which platforms break?", and fail-fast answers "one did, and we won't say which of the others would have". A Linux-only policy step took all nine jobs down with it, so three Windows and three macOS jobs reported red without finishing their tests — and every cross-platform break this repository has shipped was green on Linux and red on Windows. Reruns of a single job do not help: siblings are re-cancelled while the run is still failing.
- **The two protected-path gates took different bypasses (`.github/workflows/jules-audit.yml`)**: the scope guard honours the `allow-protected-paths` label documented in `AGENTS.md`; the self-audit only read `JULES_ALLOW_COMMAND_FILE_CHANGES`, which nothing ever set. A reviewer following the documented flow labelled the pull request and the job stayed red with no way through. The workflow now maps the label onto the variable. Doing it there rather than in `scripts/jules-self-audit.mjs` matters because `scripts/` ships in the npm package: the script keeps a single bypass mechanism and adopters do not inherit a second one. The bypass stays narrow either way — `allowProtected` skips only the protect check in `src/security.mjs`; deny rules, payload limits and the secret scanner all still run.

#### Changed
- **The risk model no longer ships one project's directory names to everyone (`src/risk.mjs`, `.agent/config.yml`)**: the builtins contained `**/vat/**`, `**/pricing/**`, `**/contracts/**`, `wrangler.jsonc`, `packages/db/**`, and this kit's own `src/engine.mjs` and `src/security.mjs` — a domain model for one installation, applied by default to every repository that installed the kit. Builtins are now what is dangerous in *any* repository: CI configuration, key material, migrations, IaC, auth, and lockfiles for ten ecosystems rather than npm's three. Domain-specific paths move to a `risk:` block in `.agent/config.yml` that **extends** the builtins rather than replacing them.
- **Shipped role prompts are stack-neutral (`.agent/prompts/`, `src/role-resolver.mjs`)**: `.agent/prompts/` ships inside the npm package, so every `agentctl init` in any language got this kit's own contribution rules — run `npm test`, run `npm run lint`, and "you are STRICTLY FORBIDDEN from adding third-party npm packages". In a Rails or Django repository that is an instruction to run a command which does not exist, plus a dependency policy nobody asked for. The prompts now use `{{VERIFY_TEST}}`, `{{VERIFY_LINT}}`, `{{DIFF_KB}}` and `{{BASE_BRANCH}}`, hydrated by `resolveRolePrompt()` from the target repository's own config; an unknown token is left visible rather than blanked, because "run  before and after" is a worse prompt than one that is diagnosable. `test/role-prompts.test.mjs` reads the shipped files and fails if `npm`, `pnpm` or `yarn` creeps back in.

#### Added
- **Retention for `.agent/evidence/` and `.agent/state/` (`src/evidence.mjs`, `src/telemetry.mjs`)**: both grew without bound in every installation. `EVIDENCE_RETENTION` keeps the newest 200 manifests, `TELEMETRY_RETENTION_DAYS` keeps 14 days. `ledger-*.jsonl` is deliberately excluded — it is the budget record, and pruning it would hand back quota that was really spent.
- **`config.source` resolves the Jules repository (`src/config.mjs`, `src/provider.mjs`)**: `createProvider` read the repository only from the task, the dispatch context, or `JULES_REPO`. A repository recorded in `.agent/config.yml` as `source:` was parsed and then ignored, so every invocation still needed the environment variable set.
- **The task wizard is covered by the fake-TTY smoke suite (`test/wizard-smoke.test.mjs`)**: it now drives the real `runTaskCreateWizard` alongside `runInitWizard`, using the exact options object `bin/agentctl.mjs` constructs — every declared key present and `undefined`. Verified against the pre-fix tree, where it fails with the message the external user reported. This is the second release in which a wizard shipped broken past a green suite; the gap both times was that no test called the function the CLI calls.
...
- **The Doc-Sync Gate Now Reports a Failing Suite as a Failing Suite (`scripts/doc-sync-check.mjs`)**: `measureTestCounts` deliberately parses the summary of a failing run, so failures showed up only as a lower pass count — reported as a README number mismatch, which reads as stale documentation and sends the reader hunting for skipped tests. A separate `test suite passes` check now names the failing tests, parsed from either the spec or TAP reporter. The gate runs where nobody is watching stdout, and a bare count costs a round-trip through CI to become actionable.
- **Full Git History in the Doc-Sync Job (`.github/workflows/jules-audit.yml`)**: The job measures test counts by running the suite, parts of which resolve a git base; `actions/checkout` defaults to depth 1, which the matrix job already overrides for the same reason.
